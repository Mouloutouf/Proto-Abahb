		...
		if(bestFit != BestFitMode.Off)
		{
			...
			if(bestFit == BestFitMode.MultilineBETA)
			{
				...
				*/
				...
				if(bestFitMulti > 1f) bestFitMulti = 1f;
				if(bestFitMulti < smallestBestFitMulti) bestFitMulti = smallestBestFitMulti; //keep it above smallest size
	...
	{ //set the data for the endmesh
		...
		for(int i=0, iL=hyphenedText.Length; i<iL; i++)
		{
			...
			if(VerticalLimitStored > 0f && (verticalLimitMode == VerticalLimitMode.ShowLast || 
				...
				lowestDrawnPositionRaw < -rawBottomRightBounds.y * (uiMode ? 2f : 1f)){ //if the bounds extend beyond the vertical limit

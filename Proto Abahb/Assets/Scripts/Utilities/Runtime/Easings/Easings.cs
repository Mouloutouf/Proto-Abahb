...
namespace Game.Utilities.Easing
{
    static public class Easing
    {
        ...
        /// <summary>
        /// Interpolate var to maxVar during duration, calling func every time it changes and using specified easing.
        /// </summary>
        ///
        public static Coroutine AutoInterpolate 
            (MonoBehaviour mono, System.Action<float, float> func, float var, float endValue, float duration, Functions easing = Functions.Linear)
        {
            return mono.StartCoroutine(AutoInterpolation(func, var, endValue, duration, easing));
        }

        private static IEnumerator AutoInterpolation(System.Action<float, float> func, float var, float endValue, float d, Functions e)
        {
            if (var == endValue) yield break;
            float elapsed = 0;
            float p = 0;
            float dif = endValue - var;
            while (elapsed <= d)
            {
                p = elapsed / d;
                func.Invoke(var + (Interpolate(p, e) * dif), p);
        ...
        public static Coroutine AutoInterpolate 
            (MonoBehaviour mono, System.Action<Vector2, float> func, Vector2 var, Vector2 endValue, float duration, Functions easing = Functions.Linear)
        {
            return mono.StartCoroutine(AutoInterpolation(func, var, endValue, duration, easing));
        }

        private static IEnumerator AutoInterpolation(System.Action<Vector2, float> func, Vector2 var, Vector2 endValue, float d, Functions e)
        {
            if (var == endValue) yield break;
            float elapsed = 0;
            float p = 0;
            ...
            {
                p = elapsed / d;
                func.Invoke(Vector2.Lerp(var, endValue, p), p);
                elapsed += Time.deltaTime;
                yield return null;
            }
            p = 1;
        ...
        public static Coroutine AutoInterpolate 
            (MonoBehaviour mono, System.Action<Vector3, float> func, Vector3 var, Vector3 endValue, float duration, Functions easing = Functions.Linear)
        {
            return mono.StartCoroutine(AutoInterpolation(func, var, endValue, duration, easing));
        }

        private static IEnumerator AutoInterpolation(System.Action<Vector3, float> func, Vector3 var, Vector3 endValue, float d, Functions e)
        {
            if (var == endValue) yield break;
            float elapsed = 0;
            float p = 0;
            ...
            {
                p = elapsed / d;
                func.Invoke(Vector3.Lerp(var, endValue, p), p);
                elapsed += Time.deltaTime;
                yield return null;
            }
            p = 1;

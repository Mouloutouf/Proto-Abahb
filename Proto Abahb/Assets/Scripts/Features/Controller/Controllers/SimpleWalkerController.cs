...
namespace Controller.Controllers
{
    ...
    public class SimpleWalkerController : Controller
    {
        ...
        private Vector3 CalculateMovementDirection()
        {
			...
			{
				_direction += tr.right * characterInput.HorizontalInput();
				_direction += tr.forward * characterInput.VerticalInput();
			}
			else
			{
				//If a camera transform has been assigned, use the assigned transform's axes for movement direction;
				//Project movement direction so movement stays parallel to the ground;
				_direction += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized * characterInput.HorizontalInput();
				_direction += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized * characterInput.VerticalInput();
			}
